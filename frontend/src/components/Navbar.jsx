...
function NavLinkAnimado({ to, icon, label, isActive }) {
  const base = `flex items-center gap-0 overflow-hidden rounded-xl px-2.5 py-1.5 font-semibold text-sm
                transition-all duration-300 group border border-transparent`
  const active = isActive
    ? 'text-[#FF006E] bg-[#FF006E]/10 border-[#FF006E]/20'
    : 'text-white/70 hover:text-white hover:bg-white/10 hover:border-white/10'

  return (
    <Link to={to} className={`${base} ${active}`}>
      <span className="text-base transition-all duration-300 group-hover:text-sm shrink-0">
        {icon}
      </span>
      <span className="max-w-0 group-hover:max-w-[120px] overflow-hidden whitespace-nowrap transition-all duration-300 group-hover:ml-1.5 text-xs font-bold">
        {label}
      </span>
    </Link>
  )
...
export default function Navbar() {
  ...
  const handleToggleApp2FA = async () => {
    if (isApp2FAEnabled) {
      const isConfirmed = await customConfirm("¿Seguro que deseas desactivar la App Autenticadora permanentemente?", "Desactivar 2FA");
      if (isConfirmed) {
        try {
          const res = await authService.disableApp2FA();
          if (res.ok) { setIsApp2FAEnabled(false); toast.success("App Autenticadora desactivada"); }
        } catch { toast.error("Error al desactivar App."); }
      }
    } else {
      try {
      ...
      } catch { toast.error("Error al iniciar setup de App."); }
    }
  };
  ...
  const handleConfirmApp = async (e) => {
    ...
    } catch { toast.error("Error verificando código."); setQrModal(prev => ({ ...prev, loading: false })); }
  };

  const handleLogout = async () => {
    const isConfirmed = await customConfirm("¿Estás seguro que deseas salir de tu cuenta?", "CERRAR SESIÓN");
    if (isConfirmed) { logout(); setMenuOpen(false); toast.success("Has cerrado sesión exitosamente."); }
  };
  ...
  const linkMobile = (path) => `block w-full rounded-xl px-4 py-3 font-semibold text-sm transition-all duration-200 ` +
    (isActive(path) ? "bg-[#FF006E]/20 text-[#FF006E] border border-[#FF006E]/40" : "text-white/90 hover:bg-white/10 hover:text-[#FFD60A]");
  ...
  // Enlaces desktop sin Mapa
  const NAV_LINKS_DESKTOP = [
    { to: '/', icon: '🏠', label: 'Inicio' },
    { to: '/catalogo', icon: '🍭', label: 'Catálogo' },
    ...(carrito.length > 0 ? [{ to: '/checkout', icon: '🛒', label: `Checkout (${carrito.length})` }] : [])
  ];

  const VENDEDOR_LINKS_DESKTOP = isVendedor ? [
    { to: '/admin/dulces', icon: '✏️', label: 'Editar Dulce' },
    { to: '/admin/dulce/nuevo', icon: '➕', label: 'Nuevo Dulce' }
  ] : [];

  // Enlaces móvil sin Mapa
  const NAV_LINKS_MOBILE = [
    { to: '/', label: '🏠 Inicio' },
    { to: '/catalogo', label: '🍭 Catálogo' },
    ...(carrito.length > 0 ? [{ to: '/checkout', label: `🛒 Checkout (${carrito.length})` }] : []),
    ...(isVendedor ? [
      ...
      <nav className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 ${navBg}`}>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative">
          <div className="flex h-16 items-center justify-between">
            {/* Logo */}
            <Link to="/" className="group flex items-center gap-2 select-none shrink-0">
            ...
            <div className="hidden md:block absolute left-1/2 -translate-x-1/2">
              <ul className="flex items-center gap-1">
                {[...NAV_LINKS_DESKTOP, ...VENDEDOR_LINKS_DESKTOP].map(({ to, icon, label }) => (
                  <li key={to}>
                    <NavLinkAnimado to={to} icon={icon} label={label} isActive={isActive(to)} />
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="hidden md:flex items-center gap-3 shrink-0">
              {isLoggedIn ? (
                <div className="flex items-center gap-3">
                  {badge && (
                    <span className={`flex items-center gap-1 px-3 py-1 rounded-full border text-xs font-bold ${badge.cls}`}>
                  ...
                  <div className="relative">
                    <button
                      ...
                      title="Seguridad"
                      className={`flex justify-center items-center h-8 w-8 rounded-full border transition-all
                        ${isEmail2FAEnabled || isApp2FAEnabled
                  ...
                  <button onClick={handleLogout} className="rounded-full border border-white/20 bg-white/5 px-4 py-2 text-sm font-bold text-white/80 hover:bg-red-500/20 hover:text-red-400 hover:border-red-500/40 transition-all hover:scale-105 active:scale-95">
                    Salir
                  </button>
                </div>
              ) : (
                <Link to="/login" className="rounded-full bg-gradient-to-r from-[#FF006E] to-[#FB5607] px-5 py-2 text-sm font-bold text-white shadow-md transition-all hover:scale-105 hover:shadow-[#FF006E]/50 active:scale-95">
                  Empezar ahora
                </Link>
              )}
            </div>
        ...
        <div className={`md:hidden overflow-hidden transition-all duration-300 ${menuOpen ? "max-h-[800px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="mx-4 mb-4 rounded-2xl border border-white/20 bg-white/10 backdrop-blur-md p-3 shadow-xl">
            <ul className="flex flex-col gap-1">
              {NAV_LINKS_MOBILE.map(({ to, label }) => (
                <li key={to}><Link to={to} className={linkMobile(to)}>{label}</Link></li>
              ))}
            </ul>
            <div className="mt-3 border-t border-white/10 pt-3">
              {isLoggedIn ? (
                <div className="flex flex-col gap-2">
                  ...
                  <button onClick={handleLogout} className="block w-full rounded-xl border border-red-500/30 bg-red-500/10 py-3 text-center text-sm font-bold text-red-400 mt-2">
                    Cerrar Sesión
                  </button>
                </div>
              ) : (
                <Link to="/login" className="block w-full rounded-xl bg-gradient-to-r from-[#FF006E] to-[#FB5607] py-3 text-center text-sm font-bold text-white">
                  Empezar ahora 🚀
                </Link>
              )}
            </div>
